...
- (void)viewDidLoad {
    ...
    for (int i = 0;i<105;i++){
        UIImage *face = [UIImage imageNamed:[NSString stringWithFormat:@"chat_face.bundle/%d.png",i]];
...
-(void)didSelectAFace:(id)sender
{
    ...
    NSString *faceStr= [NSString stringWithFormat:@"%@",[temparray objectAtIndex:0]];
     [[NSNotificationCenter defaultCenter] postNotificationName:@"FALCE_SELECT_NOTIFCATION" object:faceStr];
    [self dismissModalViewControllerAnimated:YES];
}
